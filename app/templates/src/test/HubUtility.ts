...
import { GuidString, Logger, OpenMode, DbResult, assert } from "@bentley/bentleyjs-core";
import { BriefcaseManager, StandaloneDb, BriefcaseIdValue } from "@bentley/imodeljs-backend";
import { Briefcase as HubBriefcase, BriefcaseQuery, HubIModel, IModelHubClient, IModelQuery, IModelCreateOptions } from "@bentley/imodelhub-client";
...
export class HubUtility {
  ...
  public static async createIModelWithBimFile(requestContext: AuthorizedClientRequestContext, projectId: GuidString, iModelName: string): Promise<GuidString> {
    // Delete any existing iModel
    try {
      const deleteIModelId: GuidString = await HubUtility.queryIModelIdByName(requestContext, projectId, iModelName);
      await IModelHost.iModelClient.iModels.delete(requestContext, projectId, deleteIModelId);
    ...
    // Create a new iModel
    const iModel: HubIModel = await new TestIModelHubProject().createIModelWithBimFile(requestContext, projectId, iModelName);
    return iModel.wsgId;
  }
}

class CreateOptions implements IModelCreateOptions {
  path = path.join(__dirname, "..", "assets/" + process.env.imjs_test_seed);
...
class TestIModelHubProject {
  ...
  public async createIModelWithBimFile(requestContext: AuthorizedClientRequestContext, projectId: string, iModelName: string): Promise<HubIModel> {
    const client = this.iModelHubClient;

    // Make sure that the seed bim file does have any briefcase defined in it. It should only standalone
    const nativeDb = new IModelHost.platform.DgnDb();
    const status = nativeDb.openIModel(new CreateOptions().path, OpenMode.ReadWrite);
    if (DbResult.BE_SQLITE_OK === status) {
      if (nativeDb.getBriefcaseId() !== BriefcaseIdValue.Standalone) {
        const result: DbResult = nativeDb.resetBriefcaseId(BriefcaseIdValue.Standalone);
        assert(DbResult.BE_SQLITE_OK === result);
    ...
    return client.iModels.create(requestContext, projectId, iModelName, new CreateOptions());
  }
